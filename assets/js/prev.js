...
/** 
  * hgh-zts Slider jQuery Plugin
  * Author: Aleksej Vukomanovic
  * Website: http://themeflection.com
  * Version: 1.0
  * Version from: 20.05.2015
  * Licence: MIT 
  */
!(function( $ ){
    "use strict";
      ...
      var screenSlide = {
        next: function(){ 
          ...
          init: function() {
            ...
            style += ID + ' #bullet-navs{height: auto; position: absolute;bottom: 0px; padding: 0 0.5%; padding-top: 2px; width: 100%;}';
            style += '</style>';
        ...
        first: function() { 
            ...
            {
	            wait = 1;
	            zis.find('.hgh-zt.active').removeClass('active');
	            if( parseInt( currActive + 1 ) <= parseInt( last ) ){
	                $('.zenith_slider:not(.hand)').find('.hgh-zt[data-index="'+ parseInt( currActive + 1 ) +'"]').addClass('active');
	            } else{
	                next = zis.find('.phone-holder').find('.hgi[data-index="0"]');
	                zis.find('.hgh-zt[data-index="0"]').addClass('active');
	            }//else
        ...
        second: function() {
            ...
            var prev = zis.find('.phone-hand').find('.hgi[data-index="'+ prevIndex +'"]');
	         ...
	         if( lastHit === 0 && wait === 0 ){
	         	wait = 1;
	            if( parseInt( currActive + 1 ) <= parseInt( last ) ){
	                 zis.find('.hgh-zt[data-index="'+ parseInt( currActive + 1 ) +'"]').addClass('active');
	                 prev.removeClass(animation['in_dir1'] + ' ' + animation['in_dir2']).addClass('active ' + animation['out_dir2']);
	                 next.addClass('active ' + animation['in_dir1']);
	                 parent.find(prev).one(animationEvent, function(event) {
	                     prev.removeClass(animation['out_dir2']);
       ...
       if( layout === 'default' ){
           ...
           style += ' #'+ ID +' .hgh-zt-title .fa{background: '+ circleColor +'; color: '+ iconColor +'}';
           style += ' #'+ ID +' .hgh-zt.active h3:after{background: '+ activeColor +'}';
           style += '#'+ ID +' .hgh-zt.active .fa{background: '+ activeColor +'}';
           style += '.zenith_slider{background: '+ bck +'}';
           ...
           parent.find('.hgi[data-index="'+ activeIndex +'"]').addClass('active fadeInLeft');
           parent.find('.hgh-zt[data-index="'+ activeIndex +'"]').addClass('active');

           var hIndex = '';
           parent.find('.hgh-zt').mouseenter(function(){
             if( !$(this).hasClass('active') && wait === 0 ){
             	  wait = 1;
                  var parent = $(this).closest('.zenith_slider');
                  parent.find('.hgh-zt.active').removeClass('active')
                  $(this).addClass('active');
                  hIndex = $(this).closest('.hgh-zt').data('index');
           ...
           if(  AP === 'on' ){
               zis.find('.hgh-zt').hover(function(){ clearInterval(play) }, function(){ play = setInterval(autoplay.first, parseInt( autoplaySpeed ) ); } );
           } 

           //stop autoplay on hover
           if( autoplayStop === 'on' ){ 
                 zis.find('.hgh-zt').mouseenter(function(){
                      clearInterval(play);
                 });
           }
          ...
          =================================*/
           ...
           //pause autoplay on hover
           if(  AP === 'on' ){
               zis.find('.hgh-zt').hover(function(){ clearInterval(play) }, function(){ play = setInterval(autoplay.second, parseInt( autoplaySpeed ) ); } );
           } 
